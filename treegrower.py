...
def draw_cell(
    ...
    for i in range(0, nlines):
        r = int(0.15 * ((age + 50) * 150.0 + random.randint(0, 50))
                / (max_age / 1.5))
        g = int(0.15 * ((age + 50) * 150.0 + random.randint(0, 50))
                / (max_age / 1.5))
        b = int(0.15 * ((age + 50) * 150.0 + random.randint(0, 50))
                / (max_age / 1.5))
        if r > 200:
            r = 200
        if g > 200:
...
class cell:
    ...
    def draw(self):
        p1 = rotate_point((-1 * self.size, -1 * self.size), (0, 0),
                          self.angle)
        p2 = rotate_point((-1 * self.size, 1 * self.size), (0, 0),
                          self.angle)
        p3 = rotate_point((1 * self.size, 1 * self.size), (0, 0),
                          self.angle)
        p4 = rotate_point((1 * self.size, -1 * self.size), (0, 0),
                          self.angle)
    ...
    def grow(self):
        ...
        if self.age < first_branch_max_age or self.age \
            < second_branch_max_age:
            chance = random.randint(0, 1000)
            if chance < second_branch_chance and self.nchildren < 2 \
                and self.age < second_branch_max_age or chance \
                < first_branch_chance and self.nchildren < 1 \
                and self.age < first_branch_max_age:
                random_angle = self.angle + deg_to_rad(-branch_angle
                        + random.randint(0, branch_angle * 2))
                tx = self.x + math.sin(self.angle) * self.size \
                    * cell_offset
                ty = self.y + -math.cos(self.angle) * self.size \
                    * cell_offset
...
def clear_the_screen():
   ...
   # screen.fill(white)
    ...
    for i in range(0, nrects):
        ...
        if i < 0.90 * nrects:
            pygame.draw.rect(screen, (10 + (nrects - i) * 4, 10
                             + (nrects - i) * 2, 90 + i), r, 0)
        else:
            pygame.draw.rect(screen, (0, 0, 0), r, 0)


add_cell(cell(screen_width / 2.0, screen_height * 0.90, 0.05, 0.0, -1))
add_cell(cell(screen_width / 4.0, screen_height * 0.90, 0.05, 0.0, -1))
add_cell(cell(3.0 * screen_width / 4.0, screen_height * 0.90, 0.05,
         0.0, -1))
